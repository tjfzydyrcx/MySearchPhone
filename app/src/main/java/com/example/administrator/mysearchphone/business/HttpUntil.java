package com.example.administrator.mysearchphone.business;
...
public class HttpUntil {
    ...
    class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Interceptor.Chain chain) throws IOException {
            ...
            Log.d("aa", String.format("Sending request %s on %s%n%s",
                    request.url(), chain.connection(), request.headers()));

            Response response = chain.proceed(request);

            long t2 = System.nanoTime();
            Log.d("aa", String.format("Received response for %s in %.1fms%n%sconnection=%s",
    ...
    private void run(boolean isPost) {
        ...
        Call mcall =  client.newBuilder().addInterceptor(new LoggingInterceptor()).build().newCall(request);
